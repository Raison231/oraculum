...
function createDesign(opts) { opts = opts || {}; var intent = String(opts.intent || "").trim(); if (!intent) return {ok: false, message: "Введи намерение"};
  ...
  return {ok: true, intent: intent, seed: seed, dr: dr, meaning: DR_MEANING[dr], action: DR_ACTION[dr], axis: ax, step21: st, kepler: kl, frequency: fr, archetype: opArchetype(dr),
    ...
    birthAlign: birth.ok ? {yourDR: birth.numbers.dr, resonance: digitalRoot(birth.numbers.dr + dr), note: birth.numbers.dr === dr ? "Прямой резонанс" : (mirrorPair(birth.numbers.dr) === dr ? "Зеркальный резонанс" : "Не��тральный")} : null,
...
function fieldRead(raw, opts) { opts = opts || {}; var p = parseInput(raw); if (p.type === "empty") return {ok: false, message: "Пусто — дата, число, ко��рдинаты, частота, цвет, время или слово."};
...
function routeScore(places) { if (!places || places.length < 2) return {ok: false, message: "Нужно минимум 2 точки"}; var total = 0, legs = [];
  ...
  var avg = Math.round(total / (places.length - 1)); return {ok: true, places: places, legs: legs, score: avg, verdict: avg >= 75 ? "МАРШРУТ В РЕЗОНАНСЕ" : (avg >= 50 ? "МАРШРУТ Р��БОТАЕТ" : "ПЕРЕСТРОЙ МАРШРУТ"), bestStop: legs.reduce(function (a, b) { return a.leg.nodeScore > b.leg.nodeScore ? a : b; })}; }
...
function spacePlan(room) { var s = String(room || "").trim(); if (!s) return {ok: false, message: "Опиши помещение"}; var seed = nameToNumber(s), dr = digitalRoot(seed); var zones = ["вход/внимание", "работа/фокус", "отдых/сон", "кухня/т��ло", "карта/планы", "окно/свет", "угол/глубина", "центр/сила", "потолок/видение"];
